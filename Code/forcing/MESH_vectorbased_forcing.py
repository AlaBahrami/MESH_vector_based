...
# Note: the input forcing is arranaged based on COMID. So, a reordering of forcing
# indices is required to match order Rank1 to RankN in MESH
n = len(basin)
ind = []
...
forc_vec = xs.Dataset(
    {
        "PR": (["subbasin", "time"], forc['PR'].values[:,ind].transpose()),
    },
    coords={
...
forc_vec['PR'].encoding['coordinates'] = 'time lon lat'
forc_vec['PR'].attrs["units"]          = forc['PR'].units
forc_vec['PR'].attrs["grid_mapping"]   = 'crs'

for n in ['FI','FB','TT','UV','P0','HU']:
...
forc_vec.attrs['history']     = 'Created on May 13, 2021'
