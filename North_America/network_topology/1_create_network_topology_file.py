...
last modified :
        11/22/2021 -- adding the warning message for centroid to avoid 
                      excitment on Graham
        06/13/2022 -- deactivate reading the river_outlet_id and check the 
                      most downstream ID
        06/20/2022 -- 1) Round tosegment and segid values from float to int. 
                      The values of the tosegments were written in float numbers 
                      in the river_shape file. This creates problems in finding 
                      segidment ids drains into downstream
        10/19/2022 -- Added the condition whether the river_outlet_id was not 
                      found. If the the ID of the most downstream segment was 
                      not defined in the controlFile, for instance in the NA 
                      application which we have mutiple outlets, we dont have 
                      to set the river_down_seg_id to zero. 
"""
#%%# modules
import os
import pandas as pd
...
controlFolder = Path('C:/Users/alb129/OneDrive - University of Saskatchewan/programing/python/vector_based_routing/summa_network_topology/0_control_files')

# Store the name of the 'active' file in a variable
controlFile = 'control_PFAF78.txt'
